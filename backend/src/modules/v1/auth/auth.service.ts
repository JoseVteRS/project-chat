...
import { InvalidCredentials } from 'src/common/exceptions/invalid-credentials.exception';
import { SocialProvider } from 'src/common/exceptions/social-provider.exception';
...
export class AuthService {
  ...
  public async register(registrationData: CreateAccountDto, req: Request) {
    try {
      const user = await this.userService.create({
        image: this.generateGravatar(registrationData.email),
  ...
  public async logout(req: Request) {
    if (req.cookies && req.cookies['refresh_token']) {
      ...
      const verifiedRefresh = await this.jwtService.verifyAsync(refreshTokenCookie, {
        secret: this.configService.get('JWT_REFRSH_SECRET_KEY'),
      });
  ...
  public async getAuthenticatedUser(email: string, password: string) {
    try {
      ...
      if (user.provider !== Providers.Local) throw new SocialProvider();
  ...
  public async confirmationAccount(user: User, token: string) {
  ...
  private async setTokens(req: Request, { accessToken, refreshToken }: { accessToken: string; refreshToken?: string }) {
    ...
    if (refreshToken) {
      req.res.cookie('refresh_token', accessToken, {
        maxAge: 1000 * 60 * 60 * 24 * 30,
        httpOnly: true,
        sameSite: true,
      });
